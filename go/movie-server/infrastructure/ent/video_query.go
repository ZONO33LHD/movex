...
func (vq *VideoQuery) QueryVideos() *VideoQuery {
	query := (&VideoClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, video.VideosTable, video.VideosPrimaryKey...),
		)
...
func (vq *VideoQuery) Clone() *VideoQuery {
	...
	return &VideoQuery{
		...
		withVideos: vq.withVideos.Clone(),
...
func (vq *VideoQuery) WithVideos(opts ...func(*VideoQuery)) *VideoQuery {
	query := (&VideoClient{config: vq.config}).Query()
...
func (vq *VideoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Video, error) {
	...
	if query := vq.withVideos; query != nil {
		if err := vq.loadVideos(ctx, query, nodes,
			func(n *Video) { n.Edges.Videos = []*Video{} },
			func(n *Video, e *Video) { n.Edges.Videos = append(n.Edges.Videos, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VideoQuery) loadVideos(ctx context.Context, query *VideoQuery, nodes []*Video, init func(*Video), assign func(*Video, *Video)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Video)
	nids := make(map[int]map[*Video]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(video.VideosTable)
		s.Join(joinT).On(s.C(video.FieldID), joinT.C(video.VideosPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(video.VideosPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(video.VideosPrimaryKey[0]))
...
func (vq *VideoQuery) ForUpdate(opts ...sql.LockOption) *VideoQuery {
...
func (vq *VideoQuery) ForShare(opts ...sql.LockOption) *VideoQuery {
	if vq.driver.Dialect() == dialect.Postgres {
		vq.Unique(false)
	}
	vq.modifiers = append(vq.modifiers, func(s *sql.Selector) {
...
func (vq *VideoQuery) Modify(modifiers ...func(s *sql.Selector)) *VideoSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (vs *VideoSelect) Modify(modifiers ...func(s *sql.Selector)) *VideoSelect {
